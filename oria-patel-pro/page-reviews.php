<?php
/**
 * Reviews Archive Template — /reviews/
...
<div class="op-container op-section">
	...
	// Build query args — only include meta_key when needed for ordering to avoid
	// an unintended INNER JOIN that would exclude reviews with no rating meta.
	...
	// meta_query must be an array (not empty string) to avoid WP_Query notice.
	if ( ! empty( $meta_query ) ) {
		$query_args['meta_query'] = $meta_query;
	}
	// meta_key is only needed when sorting by meta value.
	if ( $meta_key ) {
